...
class TickSliderItem(GraphicsWidget):
    ## public class
    """**Bases:** :class:`GraphicsWidget <pyqtgraph_viff.GraphicsWidget>`
    ...
    def mouseClickEvent(self, ev):
        if ev.button() == QtCore.Qt.LeftButton and self.allowAdd:
            ...
            pos.setX(min(max(pos.x(), 0), self.length))
            self.addTick(pos.x()/self.length)
        elif ev.button() == QtCore.Qt.RightButton or ev.button() == QtCore.Qt.LeftButton:
            self.showMenu(ev)
    ...
    def setTickColor(self, tick, color):
        ...
        **Arguments:**
        ...
        color           The color to make the tick. Can be any argument that is valid for
                        :func:`mkBrush <pyqtgraph_viff.mkBrush>`
        ==============  ==================================================================
        """
    ...
    def listTicks(self):
        ...
        sortList(ticks, lambda a,b: cmp(a[1], b[1]))  ## see pyqtgraph_viff.python2_3.sortList
        return ticks


class ColorMapItem(TickSliderItem):
    """
    **Bases:** :class:`TickSliderItem <pyqtgraph_viff.TickSliderItem>`

    An item that can be used to define a color gradient. Implements common pre-defined gradients that are
    customizable by the user. :class: `GradientWidget <pyqtgraph_viff.GradientWidget>` provides a widget
    ...
    **Signals:**
    ...
    def __init__(self, *args, **kargs):
        ...
        All arguments are passed to :func:`TickSliderItem.__init__ <pyqtgraph_viff.TickSliderItem.__init__>`
        ...
        **Arguments:**
        ...
        tickPen          Default is white. Specifies the color of the outline of the ticks.
                         Can be any of the valid arguments for :func:`mkPen <pyqtgraph_viff.mkPen>`
        ===============  =================================================================================
        """
    ...
    def restoreState(self, state):
        ...
        **Arguments:**
        state           A dictionary with same structure as those returned by
                        :func:`saveState <pyqtgraph_viff.ColorMapItem.saveState>`
